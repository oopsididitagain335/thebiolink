...
// --- Render Block Based on Type and Styling ---
        ...
        <div key={section.id} className={`bg-white/10 backdrop-blur-md rounded-xl p-4 shadow-lg border border-white/20 ${baseClasses}`} style={style}>
          ...
          {widget.type === 'form' && (
            <form className="space-y-3">
              ...
              <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2 rounded font-medium">
                Send Message
              </button>
            </form>
          )}
          ...
          {widget.type === 'calendar' && (
            <div className="bg-gray-800/50 p-3 rounded text-center text-gray-300">
...
export async function generateMetadata({ params }: { params: Promise<{ username: string }> }) {
  ...
  const { username } = resolvedParams;
  try {
    const userData = await getUserByUsername(username, '0.0.0.0');
