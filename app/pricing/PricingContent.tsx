...
const PLANS: Plan[] = [
  ...
  },
  {
    ...
    features: ['Unlimited BioLinks', 'Custom CSS/JS', 'Unlimited links', 'Real-time analytics', 'API access', 'White-label'],
  ...
  },
  {
    ...
    features: ['Everything in Premium', 'Early access', 'Name in credits', 'Exclusive badge', 'Eternal gratitude'],
    cta: 'Be a Fwiend',
  },
];
...
/* ==============================================================
   ...
   ============================================================== */
const HALLOWEEN_MODE = true; // ← Change to `false` to disable
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              <Link href="/" className={`px-3 py-2 rounded-lg text-sm font-medium ${c.navLink} transition-all`}>
                Home
              </Link>
              <Link href="/auth/login" className={`px-3 py-2 rounded-lg text-sm font-medium ${c.navLink} transition-all`}>
                Login
              </Link>
              <Link href="/auth/signup" className={`px-3 py-2 rounded-lg text-sm font-medium ${c.navLink} transition-all`}>
                Signup
              </Link>
              <Link
                ...
                className={`px-3 py-2 rounded-lg text-sm font-medium ${HALLOWEEN_MODE ? 'text-purple-300 hover:text-white hover:bg-purple-900/30' : 'text-indigo-300 hover:text-white hover:bg-indigo-900/30'} transition-all`}
              >
                Discord
              </Link>
            </div>
      ...
      <main className="pt-24 pb-16 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <AnimatePresence>
            {error && (
              <motion.div
                ...
                exit={{ y: -20, opacity: 0 }}
                className={`mb-8 p-4 ${c.errorBg} border rounded-xl text-center font-medium`}
              >
                {error}
              </motion.div>
            )}
          ...
          >
            <h1 className={`text-5xl sm:text-6xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r ${c.title} mb-4`}>
              Choose Your Plan
            </h1>
            <p className={`text-lg ${c.subtitle} max-w-2xl mx-auto`}>
              {HALLOWEEN_MODE ? 'Pick your potion. Brew your link. Own the night.' : 'Simple, transparent pricing. No hidden fees.'}
            </p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {PLANS.map((plan, i) => (
              <motion.div
                ...
                whileHover={{ y: -8 }}
                className={`relative ${c.cardBg} ${c.cardHover} rounded-2xl p-6 transition-all duration-300`}
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <p className={`mt-2 text-sm ${c.text}`}>{plan.description}</p>

                  <div className="mt-6">
                    <span className="text-4xl font-bold text-white">
                      {plan.price === 0 ? 'Free' : `£${plan.price}`}
                    </span>
                    {plan.price > 0 && <span className={`text-sm ${c.text}`}>/month</span>}
                  </div>

                  <ul className="mt-6 space-y-3 text-left">
                    {plan.features.map((feature, idx) => (
                      <li key={idx} className={`flex items-center text-sm ${c.text}`}>
                  ...
                  >
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 rounded-xl ${c.input} focus:outline-none focus:ring-2 transition-all mb-3`}
                    />
          ...
          <div className="text-center mt-16">
            <p className={`text-sm ${c.text}`}>
              ...
              <code className="font-mono text-indigo-400 bg-white/5 px-2 py-1 rounded">
